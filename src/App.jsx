...
export default function App() {
  ...
  const parseCSV = (text) => {
    ...
    if (rows.length > 1) {
      return rows.slice(1).filter(r => r.length > 5).map((row, index) => {
        // Normalize Group Name (Auto-fix inconsistent names)
        let groupName = row[3];
        if (groupName && groupName.trim() === 'แผนปฏิบัติราชการ') {
          groupName = 'กลุ่มแผนปฏิบัติราชการ';
        }
    ...
    <div key={idx} className="bg-white dark:bg-slate-700/50 p-5 rounded-xl border border-slate-200 dark:border-slate-600 shadow-sm hover:shadow-md transition-shadow relative overflow-hidden mb-4">
        ...
        <div className="flex flex-col md:flex-row justify-between mb-4 pl-3">
            ...
            <div className="mt-4 md:mt-0 flex flex-row md:flex-col items-center md:items-end gap-3 md:gap-1 text-right">
                <div className="flex items-center space-x-2">
                    ...
                    <div className="flex flex-col items-center bg-indigo-50 dark:bg-indigo-900/40 border border-indigo-100 dark:border-indigo-800 px-3 py-1.5 rounded-lg">
                    <span className="text-[10px] text-indigo-400 font-semibold uppercase">Actual</span>
                    <span className="text-sm font-bold text-indigo-700 dark:text-indigo-300">{course.actual}</span>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-xs pt-4 mt-4 border-t border-slate-100 dark:border-slate-600 pl-3">
            <div>
                ...
                <p className="text-slate-400 dark:text-slate-500 mt-1">งบประมาณ: {course.budget > 0 ? `${Number(course.budget).toLocaleString()} บ.` : '-'}</p>
            </div>
            <div>
  ...
  return (
    <div className="flex h-screen bg-slate-50 dark:bg-slate-900 font-sans text-slate-800 dark:text-slate-100 overflow-hidden relative transition-colors duration-200">
        ...
        <header className="h-16 bg-white dark:bg-slate-800 border-b border-slate-200 dark:border-slate-700 flex items-center justify-between px-6 shadow-sm z-10 transition-colors duration-200">
          ...
          <div className="flex items-center space-x-4">
             <div className="hidden md:flex flex-col text-right mr-2">
                <p className="text-sm font-semibold text-slate-700 dark:text-slate-200">Google Sheets Data</p>
                ...
                <div className="space-y-6 max-w-7xl mx-auto pb-10">
                  ...
                  <div className="bg-white dark:bg-slate-800 p-6 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-700 mt-6">
                     <div className="flex justify-between items-center mb-4">
                        ...
                        {clickedTopic && (
                           <button 
                             ...
                             className="text-sm flex items-center text-red-500 hover:text-red-700 bg-red-50 dark:bg-red-900/30 dark:hover:text-red-300 px-3 py-1.5 rounded-lg transition-colors border border-transparent dark:border-red-900"
                           >
                     ...
                     <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        ...
                        {Object.keys(topicStats).length === 0 && (
                           <div className="col-span-full text-center text-slate-400 py-8 border-2 border-dashed border-slate-200 dark:border-slate-600 rounded-xl">
                  ...
                  <div className="mt-8">
                    <div className="flex items-center justify-between mb-4">
                       <h3 className="text-lg font-bold text-slate-800 dark:text-white flex items-center">
                         <List className="mr-2 text-indigo-600 dark:text-indigo-400" size={24} />
                         ทำเนียบบุคลากร (Personnel Directory)
                         {clickedTopic && <span className="ml-2 text-sm font-normal text-indigo-600 dark:text-indigo-300 bg-indigo-50 dark:bg-indigo-900/30 px-2 py-0.5 rounded-md border border-indigo-100 dark:border-indigo-800">กรองตามหัวข้อ: "{clickedTopic}"</span>}
                         <span className="ml-2 text-sm font-normal text-slate-500 dark:text-slate-400 hidden sm:inline">| คลิกที่แถวเพื่อดูรายละเอียด</span>
                       </h3>
                    </div>
                          ...
                          <tbody className="divide-y divide-slate-100 dark:divide-slate-700">
                            {displayedPeopleList.length > 0 ? (
                              displayedPeopleList.map((person, idx) => (
                                ...
                                >
                                  <td className="px-6 py-4 text-slate-400 dark:text-slate-500 font-medium">{idx + 1}</td>
                                  <td className="px-6 py-4">
                                    <div className="flex items-center space-x-3">
                                      <div className="h-8 w-8 bg-indigo-100 dark:bg-indigo-900 rounded-full flex items-center justify-center text-indigo-600 dark:text-indigo-300 font-bold text-xs">
                                        {person.name.charAt(0)}
                                      </div>
                                      <span className="font-semibold text-slate-800 dark:text-white group-hover:text-indigo-700 dark:group-hover:text-indigo-300">{person.name}</span>
                                    </div>
                                  </td>
                                  ...
                                  <td className="px-6 py-4 text-center">
                                    <span className="font-bold text-indigo-600 dark:text-indigo-300 bg-indigo-50 dark:bg-indigo-900/40 px-3 py-1 rounded-full">
                                      {person.courses.length}
                                    </span>
                                  </td>
                                  <td className="px-6 py-4 text-center">
                                    <button className="text-slate-400 dark:text-slate-500 group-hover:text-indigo-500 dark:group-hover:text-indigo-400 transition-colors">
                          ...
                          {filteredData.length > 0 ? (
                            filteredData.map((item, idx) => (
                              <tr key={idx} className="hover:bg-slate-50 dark:hover:bg-slate-700/30 transition-colors">
                                ...
                                <td className="px-6 py-4 text-slate-600 dark:text-slate-300">{item.position}</td>
                                <td className="px-6 py-4 text-slate-600 dark:text-slate-300 max-w-xs truncate" title={item.topic}>{item.topic}</td>
                                <td className="px-6 py-4 text-center">
                                    <span className="inline-block px-2 py-1 rounded bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-300 font-bold">{item.target}</span>
                                </td>
                                <td className="px-6 py-4 text-center">
                                    <span className="inline-block px-2 py-1 rounded bg-indigo-50 dark:bg-indigo-900/40 text-indigo-600 dark:text-indigo-300 font-bold">{item.actual}</span>
                                </td>
                                <td className="px-6 py-4 text-center">
                                  {Number(item.gap) > 0 ? (
                                    <span className="text-red-500 dark:text-red-400 font-bold">-{item.gap}</span>
                                  ) : (
                                    <span className="text-emerald-500 dark:text-emerald-400 font-bold">OK</span>
                                  )}
                                </td>
                                <td className="px-6 py-4 text-slate-600 dark:text-slate-300">
                                    <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-blue-50 dark:bg-blue-900/30 text-blue-800 dark:text-blue-300">
                                      {item.startMonth}
                                    </span>
                                </td>
                              </tr>
      ...
      {selectedPerson && (
        <div className="fixed inset-0 bg-slate-900/60 dark:bg-slate-900/80 z-50 flex items-center justify-center p-4 backdrop-blur-sm transition-all animate-in fade-in duration-200">
          <div className="bg-white dark:bg-slate-800 rounded-2xl w-full max-w-4xl max-h-[90vh] overflow-hidden shadow-2xl flex flex-col scale-100 animate-in zoom-in-95 duration-200 border border-slate-100 dark:border-slate-700">
             
             {/* Modal Header */}
             <div className="p-6 border-b border-slate-100 dark:border-slate-700 flex justify-between items-start bg-gradient-to-r from-slate-50 to-white dark:from-slate-800 dark:to-slate-800/10">
                <div className="flex items-start space-x-5">
                   <div className="h-20 w-20 bg-indigo-600 rounded-2xl shadow-lg shadow-indigo-200 dark:shadow-none flex items-center justify-center text-white text-3xl font-bold border-4 border-white dark:border-slate-700">
                      {selectedPerson.name.charAt(0)}
                   </div>
                   <div className="pt-1">
                      <h2 className="text-2xl font-bold text-slate-800 dark:text-white">{selectedPerson.name}</h2>
                      <p className="text-slate-600 dark:text-slate-300 font-medium">{selectedPerson.position}</p>
                      <div className="flex flex-wrap items-center mt-2 text-sm text-slate-500 dark:text-slate-400 gap-3">
                         <span className="flex items-center bg-slate-100 dark:bg-slate-700 px-2 py-1 rounded-md"><Briefcase size={14} className="mr-1.5 text-indigo-500 dark:text-indigo-400"/> {selectedPerson.group}</span>
                         {selectedPerson.evaluator && (
                           <span className="flex items-center bg-slate-100 dark:bg-slate-700 px-2 py-1 rounded-md"><UserCheck size={14} className="mr-1.5 text-indigo-500 dark:text-indigo-400"/> ผู้ประเมิน: {selectedPerson.evaluator}</span>
                         )}
                      </div>
                   </div>
                </div>
             ...
             {/* Modal Body */}
             <div className="p-6 overflow-y-auto bg-slate-50/50 dark:bg-slate-900/50 custom-scrollbar dark:custom-scrollbar-dark">
                
                {/* --- Group 1: Knowledge (With 1.x numbering) --- */}
                {selectedPerson.courses.some(c => c.devType.includes('ความรู้')) && (
                   <div className="mb-8">
                       <h3 className="flex items-center text-lg font-bold text-slate-800 dark:text-white mb-4 pb-2 border-b border-blue-200 dark:border-blue-900">
                          <Book className="mr-2 text-blue-600 dark:text-blue-400" />
                          1. ความรู้ (Knowledge)
                          <span className="ml-3 bg-blue-100 dark:bg-blue-900 text-blue-700 dark:text-blue-300 text-xs px-2 py-0.5 rounded-full">
                ...
                {selectedPerson.courses.some(c => c.devType.includes('ทักษะ')) && (
                   <div className="mb-8">
                       <h3 className="flex items-center text-lg font-bold text-slate-800 dark:text-white mb-4 pb-2 border-b border-pink-200 dark:border-pink-900">
                          <PenTool className="mr-2 text-pink-600 dark:text-pink-400" />
                          2. ทักษะ (Skills)
                          <span className="ml-3 bg-pink-100 dark:bg-pink-900 text-pink-700 dark:text-pink-300 text-xs px-2 py-0.5 rounded-full">
                ...
                {selectedPerson.courses.some(c => c.devType.includes('สมรรถนะ')) && (
                   <div className="mb-8">
                       <h3 className="flex items-center text-lg font-bold text-slate-800 dark:text-white mb-4 pb-2 border-b border-emerald-200 dark:border-emerald-900">
                          <Medal className="mr-2 text-emerald-600 dark:text-emerald-400" />
                          3. สมรรถนะ (Competency)
                          <span className="ml-3 bg-emerald-100 dark:bg-emerald-900 text-emerald-700 dark:text-emerald-300 text-xs px-2 py-0.5 rounded-full">
                ...
                {selectedPerson.courses.some(c => !c.devType.includes('ความรู้') && !c.devType.includes('ทักษะ') && !c.devType.includes('สมรรถนะ')) && (
                   <div className="mb-8">
                       <h3 className="flex items-center text-lg font-bold text-slate-800 dark:text-white mb-4 pb-2 border-b border-slate-200 dark:border-slate-700">
                          <FileText className="mr-2 text-slate-600 dark:text-slate-400" />
                          อื่นๆ (Others)
                       </h3>
                       <div className="space-y-4">
